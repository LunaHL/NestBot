      ...
      for (const entry of Object.values(data.birthdays[guildId])) {
        if (entry.date === today) {
          if (entry.userId) {
            ...
            channel.send(
              `🎉 Happy Birthday <@${entry.userId}>! 🎂 You received **200 NestCoins**!`,
            );
          } else {
    ...
    .addSubcommand(sub =>
      sub
        ...
        .addUserOption(opt =>
          opt
            .setName('user')
            .setDescription('User to add birthday for (admins only)'),
        )
        .addStringOption(opt =>
  ...
  async execute(interaction) {
    ...
    if (sub === 'remove') {
      ...
      db.perform(data => {
        ...
        if (!entry) {
          return interaction.reply({
            content: `❌ Birthday #${id} does not exist.`,
            flags: 64,
          });
        }
