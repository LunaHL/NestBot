...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub => 
      sub.setName('add-core')
        .setDescription('Add a global core memory (Admin only)')
        .addStringOption(opt => opt.setName('fact').setDescription('The global fact').setRequired(true))
    )
    .addSubcommand(sub => 
      sub.setName('list-core')
        .setDescription('See global core memories')
    )
    .addSubcommand(sub => 
      sub.setName('clear-core')
        .setDescription('Clear global core memories (Admin only)')
    )
    .addSubcommand(sub => 
      sub.setName('list-all')
  ...
  async execute(interaction) {
    ...
    if (sub === 'list-core') {
      ...
      if (core.length === 0) {
        return interaction.reply({ content: "I don't have any core memories yet.", flags: 64 });
      }
      return interaction.reply({ content: `**Core Memories (Global Facts):**\n- ${core.join('\n- ')}`, flags: 64 });
    }
    ...
    if (sub === 'clear-core') {
      ...
      return interaction.reply({ content: '🧠 Core memories wiped.', flags: 64 });
    }
    ...
    if (sub === 'list-all') {
      ...
      const validEntries = entries.filter(([_, mems]) => mems && mems.length > 0);

      if (validEntries.length === 0) {
        return interaction.reply({ content: "I don't remember anything about anyone.", flags: 64 });
      }
