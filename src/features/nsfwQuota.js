...
function statePath(guildId) {
  return path.join(DATA_DIR, `nsfwQuota.${guildId}.json`);
}
...
function defaultState() {
  return {
    ...
    lastSetBy: null,
    setAt: 0,
    submissiveRoleId: null,
    mistressRoleId: null,
    nsfwChannelId: null,
    calloutChannelId: null,
  };
}
...
function saveState(guildId, s) {
  ensureDir();
  fs.writeFileSync(statePath(guildId), JSON.stringify(s, null, 2));
}
...
function nextDeadline(window, tz = "Europe/Berlin") {
  ...
  try {
    ...
    if (window === "daily") return base + 24 * 60 * 60 * 1000;
    ...
    if (window === "weekly") {
      ...
      return base + (daysToMon || 7) * 24 * 60 * 60 * 1000;
    }

    return base + 7 * 24 * 60 * 60 * 1000; // oneoff default 7 days
  } catch {
...
function resolveGuildBits(guild, state) {
  ...
  const nsfwChan = state.nsfwChannelId
    ...
    : guild.channels.cache.find(
        (c) => c.type === ChannelType.GuildText && c.name.toLowerCase() === "nsfw-picture-fun"
      );
...
function imagesInMessage(message) {
  ...
  for (const emb of message.embeds) {
    if (emb?.image?.url) count++;
    else if (emb?.thumbnail?.url) count++;
  }
  return count;
}
...
const command = new SlashCommandBuilder()
  ...
  .addSubcommand((sc) =>
    sc
      ...
      .addIntegerOption((o) =>
        o.setName("amount").setDescription("Target number of images").setMinValue(1).setRequired(true)
      )
      .addStringOption((o) =>
        o
          .setName("window")
          .setDescription("daily | weekly | oneoff")
          .addChoices(
            { name: "daily", value: "daily" },
            { name: "weekly", value: "weekly" },
            { name: "oneoff", value: "oneoff" }
          )
      )
  )
  .addSubcommand((sc) =>
...
function deadlineText(ms) {
  return ms ? `<t:${Math.floor(ms / 1000)}:R>` : "not set";
}
...
export function setupNsfwQuota(client) {
  ...
  client.once("ready", async () => {
    try {
      if (process.env.GUILD_ID) {
        await registerCommandsForGuild(client, process.env.GUILD_ID);
      } else {
        // register to all currently cached guilds (dev-friendly)
        await Promise.all(
          client.guilds.cache.map((g) => registerCommandsForGuild(client, g.id))
        );
      }
      // start ticker after registration
      startTicker(client);
      console.log("[nsfwQuota] commands registered & ticker started");
    } catch (e) {
      console.error("[nsfwQuota] failed to register commands:", e);
    }
  });
  ...
  client.on("interactionCreate", async (interaction) => {
    try {
      ...
      const { mistress, submissive, nsfwChan } = resolveGuildBits(guild, state);
      const sub = interaction.options.getSubcommand();
      const isMistress = mistress && interaction.member?.roles?.cache?.has(mistress.id);

      if (sub === "status") {
        const embed = new EmbedBuilder()
          .setTitle("📸 Submissive Picture Quota")
          .setDescription(
            `Channel: ${nsfwChan ? `<#${nsfwChan.id}>` : "*not configured*"}\n` +
            `Submissive role: ${submissive ? `<@&${submissive.id}>` : "*not configured*"}`
          )
          .addFields(
            ...
            { name: "Remaining", value: String(Math.max(0, state.target - state.progress)), inline: true },
          )
      ...
      if (sub === "config") {
        if (!isMistress) {
          await interaction.reply({ content: "Only **Mistress** may configure the quota.", ephemeral: true });
          return;
        }
      ...
      if (sub === "set") {
        if (!isMistress) {
          await interaction.reply({ content: "Only **Mistress** may set the quota.", ephemeral: true });
          return;
        }
        if (state.target > 0 && state.deadline && now() < state.deadline) {
          await interaction.reply({
            content: "A quota is already active for this window. Use `/nsfw-quota reset` to override.",
            ephemeral: true,
          });
          return;
        }
      ...
      if (sub === "reset") {
        if (!isMistress) {
          await interaction.reply({ content: "Only **Mistress** may reset the quota.", ephemeral: true });
          return;
        }
        Object.assign(state, {
          target: 0,
  ...
  client.on("messageCreate", async (message) => {
    try {
      ...
      if (count > 0) {
        ...
        saveState(message.guild.id, state);
      }
    } catch (e) {
    ...
    try {
      for (const guild of client.guilds.cache.values()) {
        ...
        if (state.progress < state.target) {
          ...
          await calloutChan.send(
            `⛓️ **Quota missed!** Submissives fell short by **${remaining}** image(s). Prepare for consequences.`
          );
          // hook punishment routine here if desired
        } else {
